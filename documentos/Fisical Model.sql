DROP DATABASE traqtomac;
CREATE DATABASE traqtomac;
...
CREATE TABLE IF NOT EXISTS pessoa (
  ...
  CONSTRAINT chk_tipo CHECK (tipo IN (0, 1)),
  CONSTRAINT chk_cpf_cnpj CHECK (
    (tipo = 0 AND cpf IS NOT NULL AND cnpj IS NULL) OR
    (tipo = 1 AND cnpj IS NOT NULL AND cpf IS NULL)
  )
);
...
CREATE TABLE IF NOT EXISTS produto (
  ...
  CONSTRAINT fk_produto_servico FOREIGN KEY (id_servico) REFERENCES servico(id_servico)
);
--COMMENT ON COLUMN produto.condicao IS '0-Pessimo 1-Mediano 2-Excelente';

CREATE TABLE IF NOT EXISTS funcionamento (
  id_funcionamento INT GENERATED BY DEFAULT AS IDENTITY,
  id_produto INT NOT NULL,
  id_maquina INT NOT NULL,
  CONSTRAINT pk_funcionamento PRIMARY KEY (id_funcionamento),
  CONSTRAINT fk_funcionamento_produto FOREIGN KEY (id_produto) REFERENCES produto(id_produto),
  CONSTRAINT fk_funcionamento_maquina FOREIGN KEY (id_maquina) REFERENCES maquina(id_maquina)
);

CREATE TABLE IF NOT EXISTS consumocompra (
  id_consumoc INT GENERATED BY DEFAULT AS IDENTITY,
  id_produto INT NOT NULL,
  id_compra INT NOT NULL,
  CONSTRAINT pk_consumocompra PRIMARY KEY (id_consumoc),
  CONSTRAINT fk_consumocompra_produto FOREIGN KEY (id_produto) REFERENCES produto(id_produto),
