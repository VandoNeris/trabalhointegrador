...
async def post_consumoservico(
    consumoservico: ConsumoServico, 
    session: AsyncSession = Depends(get_session),
    current_user: Usuario = Depends(get_current_user([TipoUsuario.ADMIN, TipoUsuario.REGULAR]))
):
...
async def put_consumoservico(
    id_consumo_servico: int, 
    consumoservico: ConsumoServico, 
    session: AsyncSession = Depends(get_session),
    current_user: Usuario = Depends(get_current_user([TipoUsuario.ADMIN, TipoUsuario.REGULAR]))
):
...
async def delete_consumoservico(
    id_consumo_servico: int, 
    session: AsyncSession = Depends(get_session),
    current_user: Usuario = Depends(get_current_user([TipoUsuario.ADMIN, TipoUsuario.REGULAR]))
):
