...
async def post_consumocompra(
    consumocompra: ConsumoCompra, 
    session: AsyncSession = Depends(get_session),
    current_user: Usuario = Depends(get_current_user([TipoUsuario.ADMIN, TipoUsuario.REGULAR]))
):
...
async def put_consumocompra(
    id_consumo_compra: int, 
    consumocompra: ConsumoCompra, 
    session: AsyncSession = Depends(get_session),
    current_user: Usuario = Depends(get_current_user([TipoUsuario.ADMIN, TipoUsuario.REGULAR]))
):
...
async def delete_consumocompra(
    id_consumo_compra: int, 
    session: AsyncSession = Depends(get_session),
    current_user: Usuario = Depends(get_current_user([TipoUsuario.ADMIN, TipoUsuario.REGULAR]))
):
